	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		...
		case WRITE_EXTERNAL_STORAGE_CODE:
			...
			if (PackageManager.PERMISSION_GRANTED != grantResults[0]) {
				...
				//如果不是第一次申请该权限 系统会在提示申请的时候 有个“不再提示的”checkbox
				//没有选择 shouldShowRequestPermissionRationale返回true  然后提示用户为什么申请权限 点击确定后重新申请
				//选择了“不再提示的”checkbox  shouldShowRequestPermissionRationale返回false  我们引导用户到应用权限页面，让用户自己手动打开

				if (ActivityCompat.shouldShowRequestPermissionRationale(mActivity, permissions[0])) {
					//提示用户为什么申请权限 点击确定后重新申请
